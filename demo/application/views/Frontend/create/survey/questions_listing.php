...
?>
	<div class="pull-right">
 		<a class="btn btn-default" href="<?php echo base_url();?>department/create/survey/question/add/<?php echo $survey_details->survey_id;?>" style="padding:3px 15px; font-size:15px;">Add Question</a>	
 		<a class="btn btn-warning" style="padding:3px 15px; font-size:15px; margin-left:5px;" href="<?php echo base_url();?>department/create/survey/templates/<?php echo $survey_details->survey_id;?>">Survey Templates </a>		
	</div>
<?php } ?>

	<div class="clearfix"></div>
	<div class="instructions" style="font-size:16px;"><strong>INSTRUCTIONS: </strong> To add skip logic, click directly on the survey question. You can only use skip logic when you configure one question per page. Add all questions in the order you desire before adding skip logic.	 </div>
	<div class="clearfix"></div>
		...
		<li id="<?php echo $questions_details->question_id;?>">
			...
			<h4>
				...
				<?php } echo 'Q'.$k.'. '.ucfirst($questions_details->question_title);}?>
					<b>
						<?php if($questions_details->question_type==1){ echo ' [Multiple Choice (OA)]';}?>
						<?php if($questions_details->question_type==7){ echo ' [Multiple Choice (MA)]';}?>
						...
						<?php if($questions_details->question_type==8){ echo ' [Rank Order]';}?>						
					</b>
				</a>
			</h4>
...
function open_model_survey_setting(survey_id,question_id){
	...
	$.ajax({url: "<?php echo base_url();?>survey/skip_logic_apply_survey_content?survey_id="+survey_id+"&question_id="+question_id, 
		beforeSend: function(){ 
			$('#skip_logic_btn'+question_id).html('<i class="fa fa-spinner fa-spin"></i>');
		},
...
function add_skip_logic(survey_id,question_id,question_type,question_priority){
	...
	$.ajax({url: "<?php echo base_url();?>survey/add_skip_logic?survey_id="+survey_id+"&question_id="+question_id+"&question_type="+question_type+"&question_priority="+question_priority, 
		beforeSend: function(){ 
			$('#new_skip_conditions').html('<h4>Please Wait &nbsp;<i class="fa fa-spinner fa-spin"></i></h4>');
		},
		success: function(result){ //alert(result);
			if(result!=''){
...
function edit_skip_logic_condition(survey_id,skip_id,question_id,question_type,question_priority){
	...
	$.ajax({url: "<?php echo base_url();?>survey/edit_skip_logic_condition?survey_id="+survey_id+"&skip_id="+skip_id+"&question_id="+question_id+"&question_type="+question_type+"&question_priority="+question_priority, 
		beforeSend: function(){ 
			$('#h_edit_skip_logic'+skip_id).html('<h4>Please Wait &nbsp;<i class="fa fa-spinner fa-spin"></i></h4>');
		},
		success: function(result){// alert(result);
			if(result!=''){
