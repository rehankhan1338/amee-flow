...
<div class="survey_heading">
	<h3><?php echo ucwords($survey_details->survey_name);?> : : Question : : Add</h3>
</div>
...
<form data-toggle="validator" class="form-horizontal" action="<?php echo base_url();?>survey/question_save" id="frm" method="post" enctype="multipart/form-data">
	<div class="col-md-12 survey_inner_question_title">
		...
		<div class="col-md-12 form-group">
			<input type="text" class="form-control required" id="survey_question_title" style="height:40px; width:70%;display: contents;" name="survey_question_title"   placeholder="Question Title" value="<?php if(isset($survey_question_fulldetails->question_title) && $survey_question_fulldetails->question_title!=''){ echo $survey_question_fulldetails->question_title;}?>" />
		</div>
	</div>	
			 ...
			 <img src="<?php echo base_url();?>assets/frontend/images/<?php echo $question_type->images;?>" alt="" />-->
				...
				<?php foreach ($master_question_type as $question_type) {?>
					<div class="col-md-12" style="margin:0; padding:0;">
						<a id="selected_question_<?php echo $question_type->id;?>" onclick="return manage_questions('<?php echo $question_type->id;?>');" class="btn btn-default question_types_btn" style="margin:10px 0; width:85%"  > 
							<?php echo $question_type->question_type;?>
						</a>
					</div>
				<?php } ?>
	 	</div>
...
<script type="text/javascript">
    jQuery( document ).ready(function() {
    	...
    	var hidden_question_id = jQuery('#hidden_question_id').val();
		
		jQuery.ajax({
			url: '<?php echo base_url(); ?>survey/edit_modal_ajax?qt='+hidden_question_type+'&qid='+hidden_question_id,
			type: 'GET',
	 		beforeSend: function(){
				jQuery('.answer_action_according_to_question').html('<img src="<?php echo base_url();?>assets/backend/images/ajax-loading.gif" class="centered"/>'); 
			},				
		...
		function manage_questions(question_type_id){
			jQuery("#hidden_question_type").val(question_type_id);	
			 if(question_type_id!=''){	 
				...
				jQuery.ajax({
					url: '<?php echo base_url(); ?>survey/get_default_question_choice?question_type='+question_type_id,
					type: 'GET',
					beforeSend: function () {
					...
					success: function (result) {  //alert(result);
						...
						//if(question_type_id==1 || question_type_id==2 || question_type_id==3){
							jQuery.ajax({
								url: '<?php echo base_url(); ?>survey/get_action_html_of_answer?question_type='+question_type_id,
								type: 'GET',
								beforeSend: function () {
