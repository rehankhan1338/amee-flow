...
<form method="post" action="<?php echo base_url();?>reflect/save_program_measurement_benchmark_tabular">
	...
	<tr class="trbg">
		...
		<th class="rowspan" style="vertical-align:middle;">PLOs</th>
		<th class="rowspan" style="vertical-align:middle;">Interventions</th>
...
<style type="text/css">
#program_checkboxes<?php echo $academic_year;?> {
  display: none;
...
#program_checkboxes<?php echo $academic_year;?> label {
  display: inline-block;padding: 3px 15px;width: 100%;color: #fff;font-size: 17px; text-align:left;
  }
#program_checkboxes<?php echo $academic_year;?> label:hover {
  background-color: #fb9337; color:#fff; border-radius:3px;
}
</style>
<script type="text/javascript">
function program_showCheckboxes<?php echo $academic_year;?>() {
  var checkboxes = document.getElementById("program_checkboxes<?php echo $academic_year;?>");
  if (!expanded) {
  	jQuery('#program_direct_assesment<?php echo $academic_year;?>').html('<option>Click Here to Hide Options</option>'); 
    checkboxes.style.display = "block";
    expanded = true;
  } else {
  	jQuery('#program_direct_assesment<?php echo $academic_year;?>').html('<option>-- Select --</option>'); 
...
<div class="multiselect">
	<div class="selectBox" onclick="program_showCheckboxes<?php echo $academic_year;?>()">
		<select class="form-control required" id="program_direct_assesment<?php echo $academic_year;?>" name="program_direct_assesment">
			<option value="">-- Select --</option>
		</select>
		<div class="overSelect"></div>
	</div>
	<div id="program_checkboxes<?php echo $academic_year;?>">	
		<?php if(isset($checklist_detail->program_DAM) && $checklist_detail->program_DAM!=''){
				$program_DAM_arr = explode(',',$checklist_detail->program_DAM);
			}else{
				$program_DAM_arr = array();
			}
			foreach($master_direct_assessment_list as $direct_assessment_details){?>
			<label for="program_<?php echo $direct_assessment_details->name;?><?php echo $academic_year;?>">
			<input type="checkbox" name="program_dam<?php echo $academic_year;?>[]" class="" id="program_<?php echo $direct_assessment_details->name;?><?php echo $academic_year;?>" <?php if(in_array($direct_assessment_details->id,$selected_dam_arr)){?> checked="checked" <?php } ?> value="<?php echo $direct_assessment_details->id;?>" /> &nbsp;&nbsp;<?php echo $direct_assessment_details->name;?></label>
		<?php } ?> 
	</div>
</div>
...
<td style="text-align:center; line-height:25px;">
<style type="text/css">
#program_checkboxes_indam<?php echo $academic_year;?> {
  display: none;
  border: 1px #dadada solid;
  position:absolute;
  width:20.7%;
...
#program_checkboxes_indam<?php echo $academic_year;?> label {
  display: inline-block;padding: 3px 15px;width: 100%;color: #fff;font-size: 17px; text-align:left;
  }
#program_checkboxes_indam<?php echo $academic_year;?> label:hover {
  background-color: #fb9337; color:#fff; border-radius:3px;
}
</style>
<script type="text/javascript">
function program_showCheckboxes_indam<?php echo $academic_year;?>() {
  var checkboxes = document.getElementById("program_checkboxes_indam<?php echo $academic_year;?>");
  if (!expanded) {
  	jQuery('#program_indirect_assesment<?php echo $academic_year;?>').html('<option>Click Here to Hide Options</option>'); 
    checkboxes.style.display = "block";
    expanded = true;
  } else {
  	jQuery('#program_indirect_assesment<?php echo $academic_year;?>').html('<option>-- Select --</option>'); 
    checkboxes.style.display = "none";
    expanded = false;
  }
}
...
<div class="multiselect">
	<div class="selectBox" onclick="program_showCheckboxes_indam<?php echo $academic_year;?>()">
		<select class="form-control required" id="program_indirect_assesment<?php echo $academic_year;?>" name="program_indirect_assesment">
			<option value="">-- Select --</option>
		</select>
		<div class="overSelect"></div>
	</div>
	<div id="program_checkboxes_indam<?php echo $academic_year;?>">
	
		<?php if(isset($checklist_detail->program_INDAM) && $checklist_detail->program_INDAM!=''){
				$program_INDAM_arr = explode(',',$checklist_detail->program_INDAM);
			}else{
				$program_INDAM_arr = array();
			}
			foreach($master_indirect_assessment_list as $direct_inassessment_details){?>
			<label for="lab_program_indam_<?php echo $direct_inassessment_details->name;?><?php echo $academic_year;?>">
			<input type="checkbox" name="program_indam<?php echo $academic_year;?>[]" id="lab_program_indam_<?php echo $direct_inassessment_details->name;?><?php echo $academic_year;?>"  <?php if(in_array($direct_inassessment_details->id,$selected_indam_arr)){?> checked="checked" <?php } ?> value="<?php echo $direct_inassessment_details->id;?>" /> &nbsp;&nbsp;<?php echo $direct_inassessment_details->name;?></label>
		<?php } ?> 
	</div>
</div>
<div style="margin:10px 0; font-weight:600;"><?php echo implode(', ',$indam_name_arr);?></div>
	...
	<td>
		...
		<textarea class="form-control" name="program_criteria_<?php echo $academic_year;?>" rows="3" style="resize:none;"  id="program_criteria_<?php echo $academic_year;?>"><?php if(isset($benchmark_tabuler_details->criteria) && $benchmark_tabuler_details->criteria!=''){ echo $benchmark_tabuler_details->criteria;}?></textarea>
	</td>
	<td><input type="text" class="form-control" name="program_sample_size<?php echo $academic_year;?>" id="program_sample_size<?php echo $academic_year;?>" value="<?php if(isset($benchmark_tabuler_details->sample_size) && $benchmark_tabuler_details->sample_size!=''){ echo $benchmark_tabuler_details->sample_size;}?>" /></td>
</tr>
...
<div>
		<input type="submit" name="save_program_measurement_benchmark_tabular" id="save_program_measurement_benchmark_tabular" value="Save & Update" class="btn btn-primary" />
	</div>
</form>