<table width="100%" cellpadding="0" cellspacing="0">
		
		<tr>
			<th rowspan="2" width="40%" style="padding: 10px; background:#485b79; color:#fff; vertical-align:middle; text-align:left;border:1px solid #ddd;"><b>PSLOS</b></td>
			<td colspan="<?php echo $rotation_plan_count;?>" style="padding: 10px; text-align:center; background:#485b79; color:#fff; vertical-align:middle;border:1px solid #ddd;"><b>Academic Year</b></td>
			<td width="10%" rowspan="2" style="padding: 10px; background:#485b79; color:#fff; vertical-align:middle; text-align:left; text-align:center; border-top:1px solid #ddd;border-bottom:1px solid #ddd;border-right:1px solid #ddd;"><b>Assessment Team</b></td>
		</tr>
		
		<tr class="trbg" style="text-align:center">
			<?php for($rpy=0;$rpy<$rotation_plan_count;$rpy++){?>
			<td style="padding: 10px; background:#485b79; color:#fff; vertical-align:middle; text-align:left; text-align:center; border-top:1px solid #ddd;border-bottom:1px solid #ddd;border-right:1px solid #ddd;" nowrap="nowrap" ><?php echo $rotation_plan_start_year+$rpy.' / '.(($rotation_plan_start_year+$rpy)+1);?></td>
			<?php } ?>
		</tr>
		
		<?php foreach($department_pslos_graduate as $graduate_pslos){
		
			$automatic_rotation_plan_details = get_automatic_rotation_plan_details_by_plso_id_h($graduate_status_value,$department_id,$graduate_pslos->id);
			if(isset($automatic_rotation_plan_details->id) && $automatic_rotation_plan_details->id!=''){
				$automatic_id = $automatic_rotation_plan_details->id;
			}else{
				$automatic_id = '';
			}
			
			?>
			<tr>
				<td style="padding: 10px;color:#333;border-bottom:1px solid #ddd;border-left:1px solid #ddd;"><?php echo ucfirst($graduate_pslos->plso_prefix.': '.$graduate_pslos->plso_title);?></td>
				<?php for($rpy=1;$rpy<=$rotation_plan_count;$rpy++){
					
					$academic_courses_details_arr = array();
					$academic_courses_details = get_automatic_rotation_plan_academic_details_h($automatic_id,$rpy);
					if(isset($academic_courses_details) && $academic_courses_details!=''){
					
						$academic_courses_details_arr = explode(',',$academic_courses_details);
						
					}else{
					
						$academic_courses_details_arr[]='';
						
					}
				
				?>
				<td style=" padding: 10px;color:#333;border-bottom:1px solid #ddd;border-left:1px solid #ddd; text-align:center; ">
 				 
					<?php // print_r($academic_courses_details_arr);
					foreach($department_courses_result_graduate as $graduate_courses_details){?>
						<?php if(in_array($graduate_courses_details->id,$academic_courses_details_arr)){
						 echo '<p style="margin:5px 0;" class="lsp5">'.$graduate_courses_details->course_prefix.' '.$graduate_courses_details->course_number.'</p>';
						  } ?> 
					<?php }?>
 				
				</td>
				<?php } ?>
				<td style="  padding: 10px;color:#333;border-bottom:1px solid #ddd;border-left:1px solid #ddd; text-align:center; border-right:1px solid #ddd;">
					 
					<?php foreach($team_members_detail_group_by as $team_members_detail){?>
						 <?php if(isset($automatic_rotation_plan_details->team_id) && $automatic_rotation_plan_details->team_id==$team_members_detail->team_id){ echo 'Team '.$team_members_detail->team_id; } } ?>
					 
					
				</td>
			</tr>
		<?php } ?>		
		
	</table>