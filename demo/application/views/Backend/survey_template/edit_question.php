<section class="content">
<div class="box">
<div class="nrow"> 
<div class="clearfix"></div> 
	<div class="col-md-12 instructions">
		<strong>Instructions:</strong> Add question title then click question type to get started.
	</div>
	<div class="clearfix"></div>
<style>
.survey_inner_question{min-height:800px;}
.survey_inner_question_title{text-align:center;}
 .contenttitle2 {
    margin: 20px 0;
    border-bottom: 2px dotted #FB9337;
    padding-bottom: 5px;
    padding-right: 20px;
    display: inline-block;
}
.contenttitle2 h3 {
    font-size: 16px;
    font-weight: normal;
    text-transform: uppercase;
    font-weight: bold;
}
.error {
    color: #a94442;
    font-size: 15px !important;
    letter-spacing: 1px;
    font-style: italic;
	padding-left:5px;
	font-weight:600;
}
#validation_status_div .error{position:absolute; margin-top:25px;}
.is_demo_question .error{position:absolute; margin-top:25px;}
table.matrix{  text-align:center;}
/*table.matrix td{ width:20%; padding:5px;}*/
table.matrix tr td:first-child{border-right: 1px solid #dedede;}
.nps span{ font-weight:600; font-size:18px;} 
.answer_action_according_to_question .form-group{ margin-bottom:5px}
</style>
<div class="survey_inner_question">
<form data-toggle="validator" class="form-horizontal" action="<?php echo base_url();?>admin/survey_template/question_save" id="frm" method="post" enctype="multipart/form-data">

	<div class="col-md-12">
		<div class="contenttitle2 nomargintop">
			<h3> Question Title</h3>
		</div> 
		<div class="col-md-12 form-group">
			<input type="text" class="form-control required" id="survey_question_title" style="height:40px; width:70%;display: contents;" 
				name="survey_question_title" placeholder="Question Title" value="<?php if(isset($default_questions_row->question_title)&&$default_questions_row->question_title!=''){echo $default_questions_row->question_title;}?>" />
		</div>
	</div>
	
	<div class="col-md-12">
		<input type="hidden" name="hidden_question_type" id="hidden_question_type" value="<?php if(isset($default_questions_row->question_type)&&$default_questions_row->question_type!=''){echo $default_questions_row->question_type;}?>" />
		<input type="hidden" name="hidden_survey_id" id="hidden_survey_id" value="<?php echo $survey_templates_details->id;?>" />
		<input type="hidden" name="hidden_choice_count" id="hidden_choice_count" value="" />

	 	<!--<div class="form-group">
			<div class="contenttitle2 nomargintop">
				<h3> Question Type</h3>
			</div> 
			<?php $master_question_type = get_master_question_type_h();?>
		 <!-- data-toggle="tooltip"  data-html="true"  data-placement="top" title="<h4>We will show design of this type of question here</h4>"
		 <img src="<?php echo base_url();?>assets/frontend/images/<?php echo $question_type->images;?>" alt="" />
			<?php foreach ($master_question_type as $question_type) {?>
				<div class="col-md-12" style="margin:0; padding:0;">
					<a id="selected_question_<?php echo $question_type->id;?>" onclick="return manage_questions('<?php echo $question_type->id;?>');" class="btn btn-default question_types_btn" style="margin:10px 0; width:85%"> <?php echo $question_type->question_type;?></a>
				</div>
			<?php } ?>
	 	</div>-->
	
		<div class="col-md-10">
			<script type="text/javascript">
				$(document).ready( function () {
					var question_type_id = jQuery("#hidden_question_type").val();
					
					if(question_type_id!=''){			 
					 	jQuery('.question_types_btn').css('background-color','#fff');
						jQuery('.question_types_btn').css('border-color','#ccc');
						jQuery('.question_types_btn').css('color','#333');
						
						jQuery('#selected_question_'+question_type_id).css('background-color','#34445e');
						jQuery('#selected_question_'+question_type_id).css('border-color','#34445e');
						jQuery('#selected_question_'+question_type_id).css('color','#f6e4a5');
					 	
						jQuery.ajax({
							url: '<?php echo base_url(); ?>admin/survey_template/get_default_question_choice_edit?question_type='+question_type_id,
							type: 'GET',
							beforeSend: function () {
								jQuery('.answer_choice_according_to_question').html('<img src="<?php echo base_url();?>assets/backend/images/ajax-loading.gif" alt="" />');
							},
							success: function (result) {  //alert(result);
			 					jQuery('.answer_choice_according_to_question').html(result);
								
								//if(question_type_id==1 || question_type_id==2 || question_type_id==3){
									jQuery.ajax({
										url: '<?php echo base_url(); ?>admin/survey_template/get_action_html_of_answer?question_type='+question_type_id,
										type: 'GET',
										beforeSend: function () {
											jQuery('.answer_action_according_to_question').html('<img src="<?php echo base_url();?>assets/backend/images/ajax-loading.gif" alt="" />');
										},
										success: function (result) {  //alert(result);
											jQuery('.answer_action_according_to_question').html(result);
										}
									});
								//}
							}
						});
						
						/*if(question_type_id==1){ // Multiple Choice Question id 1
							
			  			}*/
						
					 }
				});
				
				/*function edit_in_place(option_id){
					var answer_choice = jQuery('#answer_choice_'+option_id).html();
					jQuery('#div_choice_'+option_id).html('<input type="text" name="" id="" value="'+answer_choice+'" class="form-control" style="width:50%;display: inline-block;"  />');
				}<div class="form-group" id="div_choice_1">
					<span id="answer_choice_1" onclick="return edit_in_place('1');">Click to write Statement 1</span>
				</div>*/
			</script>
		 	
		 	<div class="form-group">
				<div class="">
					<h3> Choice Options</h3>
				</div> 
		  	</div>
			
			<div class="answer_choice_according_to_question"></div>
		</div>

		<div class="col-md-2">
			<div class="form-group">
				<div class="contenttitle2 nomargintop">
					<h3> Settings</h3>
				</div> 
			</div>
			
			<div class="answer_action_according_to_question"></div>	
		</div>
	</div>
	
		<div class="clearfix"></div>
		<div class="col-md-4">&nbsp;</div>
		<div class="col-md-4">
			<button type="submit" class="btn btn-primary" name="submit_login" onclick="return add_question();" style="width:100%; margin-top:20px;">Save and Update</button>
		</div>
	
	<script type="text/javascript">
		function add_question(){		
			var survey_question_title = jQuery("#survey_question_title").val();
			var hidden_question_type = jQuery("#hidden_question_type").val();
			if(hidden_question_type==1){
				var choice_count = jQuery("#choice_count").html();
				jQuery("#hidden_choice_count").val(choice_count);
			}
			if(survey_question_title!=''){
				if(hidden_question_type==0){
					alert('please select the question type!');return false;
				}
			}
		}
	</script>
	
<div class="col-md-4">&nbsp;</div>
</form>
</div> 
<div class="clearfix"></div>
</div> 
</div>
</section>