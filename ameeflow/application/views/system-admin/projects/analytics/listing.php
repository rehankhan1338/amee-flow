           ...
           if(count($projectArr)>0){
            foreach($projectArr as $pro){ ?>  	 
                <div class="col-4 my-3">
                    <div style="background: <?php echo $pro['bgColor'];?>;" class="project-box" onclick="return viewProTask('<?php echo $pro['proencryptId'];?>','<?php echo $pro['projectId'];?>');">
                        <h3 style="color: <?php echo $pro['fontColor'];?>;"><?php echo $pro['projectName'];?></h3>
                        <p style="color: <?php echo $pro['fontColor'];?>;">Term/Year: <label><?php echo $this->config->item('terms_assessment_array_config')[$pro['termId']]['name'].' - '.$pro['year']; ?></label> </p>
                        <p style="color: <?php echo $pro['fontColor'];?>;">Created On: <label><?php echo date('m/d/Y, h:i A',$pro['onTime']);?></label> </p>
                        <div class="hover-arrow" id="tlnk<?php echo $pro['projectId'];?>"> <i style="color: <?php echo $pro['fontColor'];?>;" class="icon-sm" data-feather="arrow-right"></i> </div>
                    </div>
                </div>
           <?php } } ?>
        
        </div>
        <?php } }else{ echo '<h5 class="mx-2 my-2">No analytics available</h5>';}?>
<script>
