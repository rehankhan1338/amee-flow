...
function manageNotification(taskId){
    $.ajax({
        type: "POST",
        url: '<?php echo base_url().$this->config->item('system_directory_name').'projects/ajaxSendNotiticationModal?tId=';?>'+taskId,
        beforeSend: function(){
            $('#resNotiModel').html('');
            $('#notiLnk'+taskId).html('<i class="fa fa-spinner fa-spin"></i>');
        },
        success: function(result, status, xhr){
            $('#sendNotiFieldSec').html(result);
            $('#sendNotiModal').modal('show');
            $('#notiLnk'+taskId).html('<i class="icon-sm" data-feather="send"></i>');
            feather.replace();
        }
    });
}
function manageProNoti(nId,taskId,projectId,notiFor){
    $.ajax({
        type: "POST",
        url: '<?php echo base_url().$this->config->item('system_directory_name').'projects/ajaxAddProNotiFrm?tId=';?>'+taskId+'&pId='+projectId+'&notiFor='+notiFor+'&nId='+nId,
        beforeSend: function(){
            $('#resNotiModel').html('');
            if(parseInt(nId)>0){
    ...
    <div class="modal-dialog modal-fullscreen"> <!-- modal-sm -->
        <div class="modal-content ">	
            <div class="modal-header lgtBlue-mh">
                <h5 class="modal-title" id="sendNotiModalTitle">Notification</h5>
