  ...
  <div class="modal-dialog">
    <div class="modal-content">	
      <div class="modal-header">
        <h5 class="modal-title" id="popMamModalLabel">Create your project</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
		<form id="popCeFrm" action="course_enrollment/manageEntry" method="post">
...
function getCourseEnrollData(val){
	if(val!=''){
		window.location = '<?php echo base_url().$this->config->item('system_directory_name').'course-enrollment?ced=';?>'+val;
	}
}
function manageMam(ceId){
    $.ajax({
        type: "POST",
        url: '<?php echo base_url().$this->config->item('system_directory_name').'course_enrollment/ajaxFields?ceId=';?>'+ceId,
        beforeSend: function(){
            if(parseInt(ceId)==0){
				$('#popCeInst').hide();
                $('#addCeBtn').prop("disabled", true);
                $('#addCeBtn').html('<i class="fa fa-spinner fa-spin"></i> Please Wait');
                $('#popMamModalLabel').html('Add your schedule of courses (SOC)');
            }else{
				$('#popCeInst').show();
                $('#popMamModalLabel').html('Upload your schedule of courses (SOC)');
				// $('#editCeBtn'+ceId).prop("disabled", true);
                $('#editCeBtn'+ceId).html('<i class="fa fa-spinner fa-spin"></i>');
            }
        },
        success: function(result, status, xhr){
            $('#manageCeFieldSec').html(result);
            $('#popMamModal').modal('show');
            if(parseInt(ceId)==0){
                $('#addCeBtn').prop("disabled", false);
                $('#addCeBtn').html('<?php echo $addBtnTxt;?>');
            }else{
				// $('#editCeBtn'+ceId).prop("disabled", false);
                $('#editCeBtn'+ceId).html('Edit');
            }
        }
    });	    
}
			...
			$.ajax({
				...
				success: function(result, status, xhr){//alert(result);
					...
					if(result_arr[0]=='success'){
						...
						window.location = site_base_url+'course-enrollment';
					}else{
