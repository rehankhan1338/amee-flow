...
class Tickets extends CI_Controller {
	...
	public function __construct(){ 
		...
		$this->data['ticketSecBaseUrl'] = base_url().$this->config->item('admin_directory_name');	
		$this->data['msg_by'] = 1;
 	}
	
	public function suggestions_bar(){
		$this->data['title']='Suggestions';	
		$this->data['page_title']='Suggestions Bar';
		$this->data['getSuggestionsArr']=$this->Tickets_mdl->getSuggestionsArr();
		//echo '<pre>'; print_r($this->data['getSuggestionsArr']);die;
		$this->load->view('Backend/includes/header',$this->data);
		$this->load->view('Backend/tickets/suggestions_bar',$this->data);
		$this->load->view('Backend/includes/footer',$this->data);  			
	}
	
	public function suggestions(){
		$this->data['title']='Suggestions';	
		$this->data['page_title']='Suggestions';
		$this->data['suggestionsList']=$this->Tickets_mdl->getSuggestionsList();
		$this->load->view('Backend/includes/header',$this->data);
		$this->load->view('Backend/tickets/suggestions',$this->data);
		$this->load->view('Backend/includes/footer',$this->data);  			
	}
	
	public function delete_suggestion(){
		if(isset($_GET['id']) && $_GET['id']!=''){
			$this->Tickets_mdl->delete_suggestion($_GET['id']);
		} 
		redirect(base_url().'admin/suggestions'); 			
	}
	
	public function index(){
		$this->data['page_title']='Contact Support : : Tickets';
		//$this->data['session_details']->role_id
		$this->data['my_tickets_listing']=$this->Tickets_mdl->admin_tickets_listing($this->data['session_details']->admin_type,'0');
		$this->data['openTickets']=$this->Tickets_mdl->openTickets();
		$this->data['closedTickets']=$this->Tickets_mdl->closedTickets();
	...
	public function conversations(){		
		...
		if(isset($unique_ticket_id) && $unique_ticket_id!=''){
			...
			if($this->form_validation->run() == FALSE){
				...
				$this->Tickets_mdl->comment_entry('1',$this->session->userdata('userid'),$ticket_id);
				redirect(base_url().'admin/tickets/conversations/'.$unique_ticket_id);
			}
