...
<div class="pull-right">
	<a class="btn btn-default" href="<?php echo base_url();?>reports/reset_time_tracker" onclick="return confirm('Resetting the Time Tracker will remove all previously time tracked activities and will leave you with no record of your assessment work. Are you sure you want to reset?');" style="padding:3px 15px;  font-size:15px;"><i class="fa fa-clock-o" aria-hidden="true"></i> Reset Time Tracker</a>
	<a class="btn btn-primary" id="time_tracker" style="padding:3px 15px; font-size:15px;margin:0 5px;"><i class="fa fa-download" aria-hidden="true"></i> Download</a>
	<a class="btn btn-default" href="<?php echo base_url();?>department/reports" style="padding:3px 15px; font-size:15px;"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to Dashboard</a>
</div>
	...
	<table width="100%" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				...
				<th nowrap="nowrap" style="padding: 10px; background:#485b79; color:#fff; vertical-align:middle; text-align:left; border-top:1px solid #ddd;border-bottom:1px solid #ddd;border-right:1px solid #ddd;">Evaluation / Assessment Activity</th>
		...
		<?php $total_time_tracker=array();foreach($time_tracker_details as $time_tracker){?>  
			<tr>
				<td nowrap="nowrap" style="padding: 10px;color:#333;border-bottom:1px solid #ddd;border-left:1px solid #ddd; font-weight:600;">
				...
				<td style="padding: 10px;color:#333;border-bottom:1px solid #ddd;border-right:1px solid #ddd;">
					<?php 
						if(isset($time_tracker->activityPerformed) && $time_tracker->activityPerformed!=''){
							echo ucwords(str_replace('_',' ',str_replace('||',' | ',$time_tracker->activityPerformed)));
						}else{
							echo '&ndash;';
						}
					 ?>
