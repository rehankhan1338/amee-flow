<table class="table table-bordered">
	<thead>
		...
		<tr style="background-color:#f5f5f5;">
			...
			<th style="vertical-align:middle; text-align:center;">What is being assessed? </th>
					...
					foreach($courses_details_data as $cdetails){
						...
						for($i=0;$i<count($course_arr);$i++){
							...
							if(isset($fCourseD[0]['course_prefix']) && $fCourseD[0]['course_prefix']!=''){
								...
								if(isset($fCourseD[0]['course_number']) && $fCourseD[0]['course_number']>0){
								$csa1[] = $fCourseD[0]['course_prefix'].' '.$fCourseD[0]['course_number'];
								}else{
									$csa1[] = $fCourseD[0]['course_prefix'];
								}
								//echo '</p>';
							}									
						}									 
