	...
	<table width="100%" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				...
				<th nowrap="nowrap" style="padding: 10px; background:#485b79; color:#fff; vertical-align:middle; text-align:left; border-top:1px solid #ddd;border-bottom:1px solid #ddd;border-right:1px solid #ddd;"><?php echo get_plsos_name_from_plso_id_h($pslos_undergraduate->pslo_number); ?>
				<br /><?php 
					$heading_years=array();
 				$course_years = get_course_years_of_plsos_h($pslos_undergraduate->pslo_number);
		...
		<?php if(count($undergraduate_student_listing)>0){ $i=0;foreach($undergraduate_student_listing as $undergraduate_student_details){?>  
			<tr>
				...
				<td nowrap="nowrap" style="padding: 10px;color:#333;border-bottom:1px solid #ddd;border-left:1px solid #ddd;border-right:1px solid #ddd;">
					...
					<?php foreach($department_pslos_undergraduate as $pslos_undergraduate){ $student_years_rating=array(); ?>
				<td nowrap="nowrap" style="padding: 10px;color:#333;border-bottom:1px solid #ddd;border-right:1px solid #ddd;">
					<?php $course_years = get_course_years_of_plsos_h($pslos_undergraduate->pslo_number);
				foreach($course_years as $course_years_details){
