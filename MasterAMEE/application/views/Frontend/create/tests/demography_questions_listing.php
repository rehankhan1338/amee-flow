...
<?php }else{?>
		<div class="pull-right">
	 		<a class="btn btn-default" href="<?php echo base_url();?>department/create/tests/question/demography_add/<?php echo $test_details->test_id;?>" style="padding:3px 7px; font-size:15px;"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add Question</a>
		</div>
		<?php } ?>
		...
		<?php if(count($dempgraphy_questions_detail)>0){
			$k=1; foreach($dempgraphy_questions_detail as $questions_details){?>		
			<li id="<?php echo $questions_details->question_id;?>">
				<span></span>
				<h4>
					<a class="que_hover" onclick="return open_model_tests_question_description('<?php echo $test_details->test_id;?>','<?php echo $questions_details->question_id;?>');"><?php if(isset($questions_details->question_title) && $questions_details->question_title!=''){ echo 'Q'.$k.'. '.ucfirst($questions_details->question_title);}?></a>
				</h4>
				<?php if(count($student_test_complete_detail)>0){}else{?>
				<div class="action_btns">
					<a href="<?php echo base_url();?>department/create/tests/demography/edit/<?php echo $questions_details->question_id;?>" style="font-size:16px;color:#3c763d;"><i class="fa fa-pencil"></i></a>				

					<a href="<?php echo base_url();?>tests/delete_test_question?question_id=<?php echo $questions_details->question_id;?>&test_id=<?php echo $test_details->test_id;?>" onclick="return confirm('Are you sure you want to delete this question?');" style="font-size:16px;color:#a94442;"><i class="fa fa-trash"></i></a>
				</div>
				<?php } ?>
			</li>
		<?php $k++;} } ?>
