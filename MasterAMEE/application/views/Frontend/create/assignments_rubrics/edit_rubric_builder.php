			...
			<tr class="append_matric_column trbg" style="text-align:center;">
				...
				<?php foreach($builder_heading_listing as $heading_details){?>
					<td class="matrix_column td_matrix_column_<?php echo $heading_details->column_no;?>">
						...
						<input type="text" name="oprf_column_<?php echo $heading_details->column_no;?>" id="oprf_column_<?php echo $heading_details->column_no;?>" value="<?php echo $heading_details->oprf_column;?>" class="form-control nxt_input required" placeholder="H" />
						<b>&ndash;</b>
						<input type="text" name="oprf_column_sec_<?php echo $heading_details->column_no;?>" id="oprf_column_sec_<?php echo $heading_details->column_no;?>" value="<?php echo $heading_details->oprf_column_sec;?>" class="form-control nxt_input required" placeholder="L" />
...
<div class="clearfix"></div>
<div class="col-md-6">	 
<table class="table table-bordered nmt20">
		<thead>
			<tr class="trbg">
				<td colspan="<?php if(count($builder_heading_listing)>0){ echo count($builder_heading_listing);}?>">
				<h4 style="font-size:18px;">Rubric Criterion</h4></td>
			</tr>
		</thead>
	<tbody>
 		<tr>
			<td>
				<ul style="margin-left:30px; line-height:30px; font-weight:600;"><?php foreach($builder_heading_listing as $heading_details){ ?>	
					<li>
						<?php if(isset($heading_details->range_name_column)&& $heading_details->range_name_column!=''){echo $heading_details->range_name_column;}?>
						&nbsp;(<?php if(isset($heading_details->oprf_column)&& $heading_details->oprf_column!=''){echo $heading_details->oprf_column;}?>
						&nbsp;-&nbsp;<?php if(isset($heading_details->oprf_column_sec)&& $heading_details->oprf_column_sec!=''){echo $heading_details->oprf_column_sec;}?>)
					</li>
				<?php } ?>
				</ul>
			</td>	
