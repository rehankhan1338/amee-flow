...
<?php if(count($deptLogicModelData)>0){ ?>
	<div class="col-md-12">
		<div class="bdr">
			<h2 style="text-align:center; text-transform:uppercase; font-weight:600; padding:10px 0">Appendix II - Logic Model</h2>
			<?php $ar = 1;foreach($deptLogicModelData as $logic_model_details){ ?>	
				
				<h4><?php echo $logic_model_details->programYear.' - '.$logic_model_details->programName;?></h4>			
				...
				<?php if($ar<count($deptLogicModelData)){ ?><hr /><?php } ?>
			<?php $ar++; } ?>
		</div>
	</div>
<?php } ?>