...
<form method="post" action="<?php echo base_url();?>design/save_manual_rotation_plan">
	<input type="hidden" name="hrotation_plan_status" id="hrotation_plan_status" value="<?php echo $graduate_status_value;?>" />
	<table class="table table-bordered mart10">
		...
		<?php foreach($department_pslos_graduate as $graduate_pslos){
			...
			$maunal_rotation_plan_details = get_maunal_rotation_plan_details_by_plso_id_h($graduate_status_value,$department_id,$graduate_pslos->id);
			...
			<tr>
				<td style="vertical-align:middle">
					<input type="hidden" name="pslos_id[]" id="pslos_id[]" value="<?php echo $graduate_pslos->id;?>" />
					<h4><i><?php echo ucfirst($graduate_pslos->plso_prefix.': '.$graduate_pslos->plso_title);?></i></h4>
				</td>
				...
				<td>
					...
					<select class="form-control" multiple="multiple" name="couses_id<?php echo $rpy.$graduate_pslos->id;?>[]" id="couses_id<?php echo $rpy.$graduate_pslos->id;?>[]" style="width:115px; padding:5px;">
						<option value="">-- select --</option>
					<?php foreach($department_courses_result_graduate as $graduate_courses_details){?>
						<option value="<?php echo $graduate_courses_details->id;?>" <?php if(in_array($graduate_courses_details->id,$academic_courses_details_arr)){?> selected="selected" <?php } ?> ><?php echo $graduate_courses_details->course_prefix.' '.$graduate_courses_details->course_number;?></option>
					<?php } ?>
					</select>
				...
				<?php } ?>
				<td  style="vertical-align:middle;">
					<select class="form-control" name="team_id<?php echo $graduate_pslos->id;?>" id="team_id<?php echo $graduate_pslos->id;?>">
						<option value="">-- select --</option>
